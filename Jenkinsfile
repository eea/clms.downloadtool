        ...
        parallel(
          ...
          "WWW": {
            node(label: 'docker') {
              script {
                try {
                  sh '''docker run -i --name="$BUILD_TAG-www" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone /debug.sh coverage -v -vv -s $GIT_NAME'''
