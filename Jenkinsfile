    ...
    stage('Tests') {
      steps {
        parallel(
          ...
          //     script {
          //       try {
          //         sh '''docker run -i --name="$BUILD_TAG-www" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="$GIT_NAME[test]" -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone /debug.sh coverage --test-path /plone/instance/src/$GIT_NAME -v -vv -S $GIT_NAME'''
          ...
          "Python3": {
            node(label: 'docker') {
              sh '''docker run -i --rm --name="$BUILD_TAG-python3" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="$GIT_NAME[test]" -e custom.cfg -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone /debug.sh coverage -v -vv -s $GIT_NAME'''
            }
          }
        )
      }
