    ...
    stage('Tests') {
      steps {
        parallel(
          ...
          "Python3": {
            node(label: 'docker') {
              sh '''docker run -i --rm --name="$BUILD_TAG-python3" -e GIT_BRANCH="$BRANCH_NAME" -e ADDONS="$GIT_NAME[test]" -e custom.cfg -e DEVELOP="src/$GIT_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone-test:5-python3 -v -vv -s $GIT_NAME'''
            }
          }
        )
      }
