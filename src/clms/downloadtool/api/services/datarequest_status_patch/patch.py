    ...
    def reply(self):
        ...
        if nuts_id:
            if bounding_box:
                ...
                return {
                    "status": "error",
                    "msg": "Error, BoundingBox is also defined"
                }
        ...
        if "Error" in response_json.get("msg", ""):
            self.request.response.setStatus(400)
            return response_json

        if "Error, task_id not registered" in response_json.get("msg", ""):
            self.request.response.setStatus(404)
            return response_json

        # pylint: disable=line-too-long
        if "Error, NUTSID and BoundingBox can't be defined in the same task" in response_json.get("msg", ""):  # noqa
            self.request.response.setStatus(400)
            return response_json
